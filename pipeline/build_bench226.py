...
Writes bench226_{reactions,metabolites,species}.json in the same schema the
...
SRC = os.path.join(HERE, "stereo_exact_significant.csv")
PROTON = "cpd00067"
WATER = "cpd00001"
...
def main():
    ...
    json.dump(reactions, open(os.path.join(HERE, "bench226_reactions.json"), "w"), indent=1)
    json.dump(mets, open(os.path.join(HERE, "bench226_metabolites.json"), "w"), indent=1)
    json.dump(species, open(os.path.join(HERE, "bench226_species.json"), "w"), indent=1)
    keep = {k: v for k, v in meta_rows.items() if k in reactions}
    with open(os.path.join(HERE, "bench226_meta.csv"), "w", newline="") as fh:
